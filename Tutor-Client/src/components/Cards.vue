...
<main class="page-content">
  <div class="card" v-for="item in size" :key="item" :style="'background-image: url('+banners[item]+')'">
    <div class="content">
      <h2 class="title">{{titles[item]}}</h2>
      <p class="copy">{{descrpitions[item]}}</p>
      <button class="btn" @click="toFather(bids[item])" >点击查看</button>
    </div>
  </div>
</main>
</template>
        ...
        if(this.all)
          this.size = res.data.length;
            ...
            this.bids.push(blog.id);
						...
						this.titles.push("暂无");
						this.users.push("暂无");
						this.banners.push("https://images.unsplash.com/photo-1545243424-0ce743321e11?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=400&fit=max&ixid=eyJhcHBfaWQiOjE0NTg5fQ");
